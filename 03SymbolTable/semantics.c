...
void buildTable(SymbolTable * table,Node * node){
    symbolTable = table;
    semanticCheck(node);
}
void semanticCheck(Node * node){
    ...
    switch(node->type){
        case(NODE_ROOT)://Basically the program
            /** handleProgram(node); */
            //TODO maybe handle the program id node and the
            //idList node
...
void handleFunction(Node * node){
    ...
    if(funcentry==NULL){
        fprintf(stderr, "Semantic Error: Redeclaration on Function %s at depth %d\n\
    ...
    closeScope(symbolTable,funcentry);//CLOSING SCOPE
...
void handleProcedure(Node * node){
    ...
    if(entry==NULL){
        fprintf(stderr, "Semantic Error: Redeclaration on Procedure %s at depth %d\n\
                We will proceed by deleting this new name from the table and leaving the old one.\n",idNode->stringVal,symbolTable->curDepth);
    }else{
...
MethodSignature* handleParameters(Node * argumentsNode,NAME_TYPE nameType){
    ...
    while(singArgumentNode !=NULL){//I think ARUGMENS should have no children if no params were found
        ...
        handleVarDeclaration(singArgumentNode);//Since scope was opened then dump all the arguments to the new symbol table
...
void handleVarDeclaration(Node * node){
    ...
    while(singleId!=NULL){
        ...
        if(entry==NULL){
            fprintf(stderr, "Semantic Error: Redeclaration on Variable %s at depth %d\n\
...
void handleRHSVarRef(Node* varrefNode){//By the time this is called its because there is an entry for it in the symbol table
    ...
    if(entry->attributes->nameClass == CLASS_FUNCTION){//This is when the function has no vars
#ifdef DEBUG
        printf("Type of argumentless function %s\n",EnumNameRef[entry->attributes->nameType]);
#endif
        ...
        printf("\tThis is an array %s\n",entry->name);
            ...
            wasThereSemanticError = true;
            char errmsg[150];
            sprintf(errmsg,"Use of RHS varibale with name:%s that has not been initialized",idNode->stringVal);
    ...
    if(wasThereSemanticError)
        ...
        varrefNode->nodeStdType = entry->attributes->nameType;
#ifdef DEBUG
        printf("Name %s of type %s referenced\n",idNode->stringVal,NameTypeStr[varrefNode->nodeStdType]);
#endif
...
void handleAssignOp(Node * assignNode){
    ...
    if(varNode->nodeStdType == TYPE_ERR ||
        ...
        printf("NOTICE: The var \"%s\" is now initialized \n",varNode->lMostChild->stringVal);
    }
}
...
#ifdef DEBUG
    printf("Type of call %s\n",EnumNameRef[funcNode->type]);
#endif
#ifdef DEBUG
    printf("Found a Function call for %s\n",idNode->stringVal);
#endif
    //Check if the function exists
