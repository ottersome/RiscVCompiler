...
void closeScope(SymbolTable* symbolTable, SymbolTableEntry* entryForClosedMethod,Node* assocNode, size_t* tabIndex){//Since opening and closing of scopes is done only on methods
    printf("NOTICE: Closing Scope as we perform semantic check\n");
    //Save Symbol Table Somewhere else
    //SymbolTable * destTable = &symbolTable[*(tabIndex)+1];
    //initializeSymbolTable(destTable);
    //memcpy(destTable->HashTable,&symbolTable[0],sizeof());
    symbolTable[*(tabIndex)+1] = symbolTable[0];
    *(tabIndex) = *tabIndex + 1;
    assocNode->assocTable=&symbolTable[*tabIndex];
    //
    ...
    //MEGA TODO: CODEGEN: Make sure that commenting this line wont have negative repercusions
    //symbolTable->scopeDisplay[symbolTable->curDepth] = NULL;
    symbolTable->curDepth--;
}
...
SymbolTableEntry* addSymbol(SymbolTable * symbolTable, char * name, Attributes* attr){//SUPER TODO think about actually copying the str value here instead of pointing to the value in the AST. Because if for some reason you delete teh AST before thsi you are in for hell bruv
    ...
    printf("Name %s gets hash %d",name,hash);
